...
int breadthFirstSearch(GRAPH*, int*, int, int);
...
int main(){
	...
	printf("%d", breadthFirstSearch(graph, label, u-1, v-1));
	
	return 0;
}

int breadthFirstSearch(GRAPH* graph, int* label, int start, int dest){
	NODE* temp;
	int head = 0, tail = 0, current, n = graph->numOfNodes;
	int* queue = (int*) malloc(n*sizeof(int));
	queue[tail++] = start;
	label[start] = 1;
	while(head < tail && queue[head] != dest){
		current = queue[head++];
	...
	free(queue);
	return (!label[dest]) ? -1 : label[dest] - 1;
}
